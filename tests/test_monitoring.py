...
class TestBusinessMetricsTracker:
    ...
    def test_error_tracking(self):
        ...
        self.tracker.track_request(
            endpoint="/predict", method="POST", status_code=500, response_time_ms=100.0
        )
    ...
    def test_error_summary(self):
        ...
        for endpoint, code in zip(error_endpoints, error_codes):
            self.tracker.track_request(
                endpoint=endpoint, method="POST", status_code=code, response_time_ms=100
            )
    ...
    def test_anomaly_detection(self):
        ...
        error_anomaly = next(
            (a for a in anomalies["anomalies"] if a["type"] == "HIGH_ERROR_RATE"), None
        )
...
class TestMonitoringAPI:
    ...
    def test_health_endpoint_with_monitoring(self, mock_app):
        ...
        with (
            patch("src.inference.api.model", Mock()),
...
class TestModelStalenessMonitoring:
    ...
    def test_fresh_model_staleness(self):
        ...
        with (
            ...
            mock_metrics.return_value = {"metrics": {"requests_per_minute": 5.0}}

            with (
                patch("src.inference.api.prediction_logger", self.logger),
    ...
    def test_stale_model_detection(self):
        ...
        with (
            patch.object(self.logger, "model_loaded_at", old_time),
            patch.object(self.logger, "get_performance_summary") as mock_summary,
            patch.object(self.tracker, "get_current_metrics") as mock_metrics,
        ):

            mock_summary.return_value = {"performance_stats": {"total_predictions": 10}}

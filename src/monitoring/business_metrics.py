...
class BusinessMetricsTracker:
    ...
    def track_request(
        ...
        # Log the request
        self.logger.info(
            json.dumps({"event_type": "request_tracked", **asdict(metrics)})
        )
            ...
            self.current_metrics.update(
                {
                    ...
                    "p95_response_time_ms": (
                        self._percentile(response_times, 0.95) if response_times else 0
                    ),
                    "p99_response_time_ms": (
                        self._percentile(response_times, 0.99) if response_times else 0
                    ),
        ...
        with self._lock:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "error_rate_percent": (
                        (len(errors) / len(endpoint_requests) * 100)
                    ...
                    "p50_response_time_ms": (
                        self._percentile(response_times, 0.5) if response_times else 0
                    ),
                    "p95_response_time_ms": (
                        self._percentile(response_times, 0.95) if response_times else 0
                    ),
        ...
        with self._lock:
            ...
            if not recent_errors:
                return {
                    "errors": [],
                    "summary": {},
                    "message": "No errors in the specified period",
                }
            ...
            return {
                "summary": {
                    ...
                    "most_common_error": (
                        max(error_by_status.items(), key=lambda x: x[1])
                        if error_by_status
                        else None
                    ),
                    "most_problematic_endpoint": (
                        max(error_by_endpoint.items(), key=lambda x: x[1])
                        if error_by_endpoint
                        else None
                    ),
                },
            ...
            anomalies.append(
                {
                    ...
                    "severity": (
                        "HIGH" if metrics["error_rate_percent"] > 10.0 else "MEDIUM"
                    ),
            ...
            anomalies.append(
                {
                    ...
                    "severity": (
                        "HIGH" if metrics["p95_response_time_ms"] > 10000 else "MEDIUM"
                    ),

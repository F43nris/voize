...
class PredictionLogger:
    ...
    def _calculate_feature_stats(
        self, input_text: str, feature_vector: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        ...
        stats = {
            ...
            "avg_word_length": (
                np.mean([len(word) for word in input_text.split()])
                if input_text.split()
                else 0
            ),
            "char_diversity": (
                len(set(input_text.lower())) / len(input_text) if input_text else 0
            ),
            "uppercase_ratio": (
                sum(1 for c in input_text if c.isupper()) / len(input_text)
                if input_text
                else 0
            ),
            "digit_ratio": (
                sum(1 for c in input_text if c.isdigit()) / len(input_text)
                if input_text
                else 0
            ),
            "punctuation_ratio": (
                sum(1 for c in input_text if not c.isalnum() and not c.isspace())
                / len(input_text)
                if input_text
                else 0
            ),
    ...
    def _update_performance_stats(self, log_entry: PredictionLog) -> None:
        ...
        self.performance_stats["avg_preprocessing_time"] = (
            self.performance_stats["avg_preprocessing_time"] * (n - 1)
            + log_entry.preprocessing_time_ms
        ) / n
        self.performance_stats["avg_prediction_time"] = (
            self.performance_stats["avg_prediction_time"] * (n - 1)
            + log_entry.prediction_time_ms
        ) / n
        self.performance_stats["avg_confidence"] = (
            self.performance_stats["avg_confidence"] * (n - 1) + log_entry.confidence
        ) / n
    ...
    def _store_for_drift_detection(self, log_entry: PredictionLog) -> None:
        ...
        if len(self.recent_predictions) > self.max_recent_predictions:
            self.recent_predictions = self.recent_predictions[
                -self.max_recent_predictions :
            ]
    ...
    def detect_drift(self, window_size: int = 100) -> Dict[str, Any]:
        ...
        if len(self.recent_predictions) < window_size:
            return {
                ...
                "message": f"Need at least {window_size} predictions",
            }
        ...
        drift_metrics["low_confidence_ratio"] = sum(
        ...
        prediction_distribution = {
            pred: predictions.count(pred) / len(predictions)
